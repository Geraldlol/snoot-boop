  ...
  return (
    <div>
      ...
      <div className="text-center mb-3">
        <button
          className="text-[10px] font-mono px-6 py-2 rounded-lg font-bold cursor-pointer"
          style={{
      ...
      {result && !spinning && (
        <div className="text-center p-2 rounded bg-black/20">
          {result.isTriple ? (
            ...
            <div className="text-[10px] font-mono text-white/40">Miss</div>
          )}
...
function RaceTab({ bp }: { bp: number }) {
  ...
  return (
    <div>
        ...
        {RACE_GEESE.map((goose) => (
          <div key={goose.id} className="flex items-center gap-2 p-2 rounded bg-black/20 border border-white/5">
            <div className="flex-1">
              ...
              <div className="text-[7px] font-mono text-white/30">Odds: {goose.odds}:1</div>
            </div>
            <input
              type="number"
      ...
      <div className="text-center mb-3">
        <button
          className="text-[10px] font-mono px-6 py-2 rounded-lg font-bold"
          style={{
  ...
  return (
    <div>
      ...
      <div className="mb-3 p-2 rounded bg-black/20 border border-white/5">
        <div className="text-[8px] font-mono text-white/30 mb-1">Wheel Segments:</div>
        <div className="grid grid-cols-2 gap-0.5">
          {WHEEL_SEGMENTS.map((seg) => (
            <div key={seg.id} className="text-[7px] font-mono text-white/40 px-1">
              {seg.name}
            </div>
          ))}
        </div>
      ...
      {activeEffects.length > 0 && (
        <div className="p-2 rounded border border-white/5 bg-black/20">
          <div className="text-[8px] font-mono text-white/30 mb-1">Active Effects:</div>
          {activeEffects.map((desc, i) => (
            <div key={i} className="text-[8px] font-mono text-[#50C878]">{desc}</div>
          ))}
        </div>
      )}
    </div>
